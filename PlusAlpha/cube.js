...
//ログ保存
var linelog = new Array();
var globali = 0;
var shufflelog = new Array();
var shufflelogi = 0;
//キューブ各面の配列
...
//効果音を設定(キューブのラインの回転時、キューブの回転時に呼び出す)
var cubelineaudio = new Audio("kati.ogg");
var cubeaudio = new Audio("putting_pencil.ogg");
//キューブの面を"時計回り"に回転させる
...
function Shuffle(){
	...
	for(var i = 0; i < 50; i++){
		...
		//Shufflelog[i] =shuffleLine;
		switch (shuffleLine){
		case 0:
			Rotation_A();
			shufflelog[shufflelogi]=0;
			shufflelogi++;
			break;
		case 1:
			Rotation_A_F();
			shufflelog[shufflelogi]=1;
			shufflelogi++;
			break;
		case 2:
			Rotation_F();
			shufflelog[shufflelogi]=2;
			shufflelogi++;
			break;
		case 3:
			
			Rotation_B();
			shufflelog[shufflelogi]=3;
			shufflelogi++;
			break;
			
		case 4:
			Rotation_B_D();
			shufflelog[shufflelogi]=4;
			shufflelogi++;
			break;
		case 5:
			Rotation_D();
			shufflelog[shufflelogi]=5;
			shufflelogi++;
			break;
		case 6:
			Rotation_C();
			shufflelog[shufflelogi]=6;
			shufflelogi++;
			break;
		case 7:
			Rotation_C_E();
			shufflelog[shufflelogi]=7;
			shufflelogi++;
			break;
		case 8:
			Rotation_E();
			shufflelog[shufflelogi]=8;
			shufflelogi++;
			break;
		case 9:
			Rotation_A();
...
function removeShuffle(){
	     //シャッフル用変数
		 	syokiA = [  blue.b0, blue.b1, blue.b2,
		   blue.b3, blue.b4, blue.b5,
		   blue.b6, blue.b7, blue.b8];
		    syokiB = [  red.r0, red.r1, red.r2,
		   red.r3, red.r4, red.r5,
		   red.r6, red.r7, red.r8];
			syokiC = [  green.g0, green.g1, green.g2,
		   green.g3, green.g4, green.g5,
		   green.g6, green.g7, green.g8];
			syokiD = [  orange.o0, orange.o1, orange.o2,
		   orange.o3, orange.o4, orange.o5,
		   orange.o6, orange.o7, orange.o8];
			syokiE = [  yellow.y0, yellow.y1, yellow.y2,
		   yellow.y3, yellow.y4, yellow.y5,
		   yellow.y6, yellow.y7, yellow.y8];
			syokiF = [  white.w0, white.w1, white.w2,
		   white.w3, white.w4, white.w5,
		   white.w6, white.w7, white.w8];
	...
	for(shufflelogi=0; shufflelogi<shufflelog.length; shufflelogi++){
	
		
		//shuffleLine = parseInt((Math.random() * 10));
		//Shufflelog[i] =shuffleLine;
		console.log= (shufflelog[shufflelogi]);
		switch (shufflelog[shufflelogi]){
		case 0:
			Rotation_A();
			break;
		case 1:
...
function start(){
	...
	sec += 0.1;
	document.getElementById("sec").innerHTML= Math.round(sec * 10)/10; },100);
}
...
var iro1= A[0].charAt[0];
var iro2= B[0].charAt[0];
var iro3= C[0].charAt[0];
var iro4= D[0].charAt[0];
var iro5= E[0].charAt[0];
var iro6= F[0].charAt[0];
...
console.log(iro1);
	for(i=0;i<9;i++){
		...
		if(A[i].charAt[0]==iro1)
		count++;
		

		
		if(B[i].charAt[0]==iro2)
		count++;
	
		if(C[i].charAt[0]==iro3)
		count++;
	
		if(D[i].charAt[0]==iro4)
		count++;
	
		if(E[i].charAt[0]==iro5)
		count++;
	if(E[i].charAt[0]==iro6)
		count++;
